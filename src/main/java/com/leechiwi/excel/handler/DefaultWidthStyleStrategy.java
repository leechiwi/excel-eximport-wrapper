...
public class DefaultWidthStyleStrategy extends AbstractColumnWidthStyleStrategy {
    ...
    private List <Integer> defaultHeadColumnWidths=new ArrayList<>();//此处代表各表头列对应注解@ColumnWidth宽度
    private List <Integer> headColumnWidths=new ArrayList<>();//此处代表各表头列对应宽度
    @Override
    protected void setColumnWidth(WriteSheetHolder writeSheetHolder, List<WriteCellData<?>> cellDataList, Cell cell, Head head, Integer relativeRowIndex, Boolean isHead) {
        boolean needSetWidth = isHead || !CollectionUtils.isEmpty(cellDataList);
        if (needSetWidth) {
            Integer columnWidth = this.dataLength(cellDataList, cell, isHead,head);
            if (columnWidth >= 0) {
                ...
                if(this.defaultHeadColumnWidths.get(cell.getColumnIndex())>0){
                    columnWidth=this.defaultHeadColumnWidths.get(cell.getColumnIndex());
                }
                writeSheetHolder.getSheet().setColumnWidth(cell.getColumnIndex(), columnWidth * 256);
            }
        }
    }
    private Integer dataLength(List<?> cellDataList, Cell cell, Boolean isHead, Head head) {
        if (isHead) {
            Integer colWidth = Optional.ofNullable(head.getColumnWidthProperty()).map(ColumnWidthProperty::getWidth).orElse(0);
            this.defaultHeadColumnWidths.add(colWidth);
